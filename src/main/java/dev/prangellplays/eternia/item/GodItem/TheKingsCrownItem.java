package dev.prangellplays.eternia.item.GodItem;
...
public class TheKingsCrownItem extends ArmorItem implements Equipment {
    ...
    public void inventoryTick(ItemStack stack, World world, Entity entity, int slot, boolean selected) {
        if (entity instanceof PlayerEntity && ((PlayerEntity) entity).getEquippedStack(EquipmentSlot.HEAD).isOf(EterniaArmour.KINGS_CROWN)) {
            ((PlayerEntity) entity).addStatusEffect(new StatusEffectInstance(StatusEffects.RESISTANCE, 20, 0, true, false, true));
        }
        else {
            return;
        }
